...
- 默认监听玩家当前区块与相邻八个区块（3×3）；玩家可在 owo-lib 中选择半径 0、1、2，服务端按 `maximumListeningChunkRadius` 裁剪并保存到玩家 CCA。
- 每个数据包保存声音发生瞬间的坐标、分类、强度、敌对标记、遮挡等级与游戏时间，不包含实体 ID、UUID、类型或持续追踪句柄。生物移动后旧声纹停留在原位置淡出，只有再次发声才产生新声纹。
- 屏幕内使用青白色或偏暖色的中心声点、破碎波形与多层扩散弧线；屏幕外使用贴边半圆声纹，不显示箭头、名称或距离。投影使用当前相机与世界投影矩阵，因此第一、第三人称共享同一位置逻辑。
- 墙后声纹仍显示，但降低透明度、增加抖动并使弧线更破碎；环境方块仍必须通过玩家视线首碰撞面检查，因此不会揭示墙后矿石。
- 方块轮廓半径可设为 0～4；对应最多 0、12、32、56、80 个外露方块，并继续使用已有深度正确的真实 `BakedModel` 与纹理 Alpha 轮廓。
- 脚步按单个生物至少间隔 5 Tick；每名玩家默认每秒最多处理 12 个普通声纹与 6 个危险声纹。导盲杖轮廓优先级保持更高。
- 不绘制生物模型、名称、血量、装备、数量、距离、箭头或雷达点；敌对生物静止未发声时仍只有独立的模糊危险提示。
...
按 `B` 或通过 Mod Menu 打开同一个 owo-lib 客户端配置界面。声音感知分类可设置监听区块半径、环境轮廓半径、屏幕外声纹、尺寸、亮度、持续时间、墙后模糊和同时显示数量；其中两个范围值会发送到服务端校验并持久化到玩家 CCA。服务端配置位于 `config/blindness-server.json5`，负责悬崖阈值、声音范围上限、事件/方块上限、敌对检测范围与冷却。
